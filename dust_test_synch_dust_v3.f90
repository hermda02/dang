program dust_fit
    ...
    bands(1) = ('test_synch_dust_010_')
    bands(2) = ('test_synch_dust_020_')
    bands(3) = ('test_synch_dust_030_')
    bands(4) = ('test_synch_dust_040_')
    bands(5) = ('test_synch_dust_050_')
    bands(6) = ('test_synch_dust_100_')
    ...
    do j = 1, nbands
        ...
        rmss(:,:,j) = rms
        call read_bintab('test_data/' // trim(bands(j)) // 'noise_map_n0008.fits', &
    ...
    do k = 1, nmaps
        ...
        do iter = 1, niter
            ...
            ! ! -------------------------------------------------------------------------------------------------------------------
            fg_amp(:,k,3,1)           = sample_s_amp((maps-dust_map),fg_amp(:,:,3,1), rmss, k, 1)
            ...
            if (mod(iter,500) .EQ. 0) then
                ...
                if (output_fg .eqv. .true.) then
                    ...
                    title = trim(direct) // trim(bands(3)) // 'synch_amplitude_' //  trim(tqu(k)) &
                            // '_' // trim(iter_str) // '.fits'
                    call write_maps(fg_amp(:,k,4,1),npix,1,title)
                end if
                do j = 1, nbands
  ...
  contains
    ...
    function sample_beta(band, npix, map_n, sigma, beta, nside2)
        ...
        do i = 0, npix2-1
            ...
            do j = 1, nbands
                a = a + (((fg_amp_low(i,map_n,3) * compute_fg(1,nuz(j),sol)) &
            ...
            do l = 1, iterations
                ...
                b = 0.d0
                do j = 1, nbands
                    tmp(j) = fg_amp_low(i,map_n,3)*compute_fg(1,nuz(j),t)

program ame_fit
    ...
    do j = 1, nbands
        call read_bintab('test_data/raw_data/' // trim(bands(j)) // 'rms_n0064.fits',rms,npix,nmaps,nullval,anynull,header=header)
        rmss(:,:,j) = rms
    end do


    do j = 1, nbands
        call read_bintab('test_data/raw_data/' // trim(bands(j)) // 'map_n0064_60arcmin.fits', &
    ...
    do k = 1, nmaps
        ...
        do iter = 1, niter
            ...
            if (mod(iter,1) .EQ. 0) then
                ...
                do j = 1, nbands
                    title =  trim(bands(j)) // 'ame_fit_'// trim(tqu(k)) // '_' // trim(iter_str) // '.fits'
                    call write_maps(ame_map(:,k,j),npix,1,title)
                    title = trim(bands(j)) // 'synch_amplitude_' //  trim(tqu(k)) // '_' // trim(iter_str) // '.fits'
  ...
  contains
    ...
    !     real(dp), dimension(0:npix-1,nmaps) :: band, temp, rms

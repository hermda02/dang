module dang_freefree_comp_mod
  use dang_component_mod
  use dang_mixmat_1d_mod
  implicit none

  private
  public dang_freefree_comp

  type, extends (dang_comp) :: dang_freefree_comp
   contains
     procedure :: S => eval
  end type dang_freefree_comp

  interface dang_freefree_comp
     procedure constructor
  end interface dang_freefree_comp
...
contains
  ...
  function constructor(dpar,component)
    ...
    class(dang_freefree_comp), pointer :: constructor
  ...
  function eval(self, nu, band, pol, theta, pixel)
    ...
    implicit none
    class(dang_freefree_comp), intent(in)         :: self
    ...
    if (present(nu)) then
       spectrum = log(exp(5.960d0 - sqrt(3.d0)/pi*log(1.d0*nu/1.d9*(T_e/1.d4)**(-1.5d0))) + 2.71828d0)/&
            & S_ref*(nu/self%nu_ref)**(-2)
...
end module dang_freefree_comp_mod

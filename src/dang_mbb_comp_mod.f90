module dang_mbb_comp_mod
  use dang_component_mod
  use dang_mixmat_2d_mod
  implicit none

  private
  public dang_mbb_comp


  type, extends (dang_comp) :: dang_mbb_comp
   contains
     procedure :: S  => eval
  end type dang_mbb_comp

  interface dang_mbb_comp
     procedure constructor
  end interface dang_mbb_comp
...
contains
  ...
  function constructor(dpar,component)
    ...
    class(dang_mbb_comp), pointer :: constructor
    ...
    do k = 1, 3
       do j = 1, nbands
          constructor%mixmat(j,k)%p => dang_mixmat_2d(constructor, bp(j), k)
       end do
    end do
  ...
  function eval(self, nu, band, pol, theta, pixel)
    ...
    implicit none
    class(dang_mbb_comp),    intent(in)           :: self
    ...
    if (present(nu)) then
       spectrum = (exp(z*self%nu_ref)-1.d0) / &
            & (exp(z*nu)-1.d0) * (nu/(self%nu_ref))**(beta+1.d0)

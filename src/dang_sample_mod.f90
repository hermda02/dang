...
contains
  ...
  subroutine sample_joint_amp(dpar, ddata, compo, map_n, method)
    !------------------------------------------------------------------------
    ...
    real(dp), allocatable, dimension(:) :: eta, dummy1, dummy2
    ...
    do n = 1, dpar%ncomp
       if (ANY(dpar%joint_comp == trim(dpar%fg_label(n)))) then
          ...
          map2fit(:,:,:) = map2fit(:,:,:) - ddata%fg_map(:,:,1:,n)
       end if
    end do
    do n = 1, dpar%ntemp
       if (ANY(dpar%joint_comp == trim(dpar%temp_label(n)))) then
          ...
          map2fit(:,:,:) = map2fit(:,:,:) - ddata%fg_map(:,:,1:,dpar%ncomp+n)
       end if
    end do
    ...
    call cg_groups(1)%p%compute_rhs(ddata,b)
    write(*,*) 'Run new CG search'
    call cg_groups(1)%p%cg_search(dpar,ddata,map_n,b)
    ...
    ! do i = 1, size(b) !2*npix
    !    eta(i) = rand_normal(0.d0,1.d0)
    ! end do

    ! allocate(dummy1(size(b)))
    ! allocate(dummy2(size(b)))

    ! write(*,*) 'Checking sample vector calculation'
    ! dummy1 = compute_sample_vec(dpar, ddata, compo, eta, nbands, 2, b)
    ! dummy2 = cg_groups(1)%p%compute_sample_vector(ddata,eta,nbands,2,b)
    
    ! write(*,*) 'Checking Ax calculation'
    ! dummy1 = return_Ax(dpar,ddata,compo,eta,nbands,2)
    ! dummy2 = cg_groups(1)%p%compute_Ax(ddata,eta,nbands,2)

    ! open(54,file='calculate_sample_vector.dat')
    ! do i = 1, size(b)
    !    write(54,fmt='(2(E16.8))') dummy1(i), dummy2(i)
    ! end do
    ! close(54)
    ! stop
    ...
    !    ! call sample_cg_vec(c,dpar,ddata,compo,map_n)
    !    call sample_cg_vec(c,b,dpar,ddata,compo,map_n)
    ! else if (trim(method) == 'cg_precond') then
    !    call sample_cg_vec_precond(b,c,dpar,ddata,compo,map_n)
    ! else
    !    write(*,*) trim(method)
    ...
    do i = 1, size(b)
       write(55,fmt='(2(E16.8))') b(i), cg_groups(1)%p%x(i)
    end do
    close(55)
    stop
       ...
       do m = 1, size(dpar%joint_comp)
          do n = 1, dpar%ncomp
             if (trim(dpar%joint_comp(m)) == trim(dpar%fg_label(n))) then
                do i = 1, x
                   ddata%fg_map(i-1,map_n,0,n) = ddata%amp_vec(w+i)!b(w+i)
                end do
                w = w + x
             end if
          end do
                ...
                do while (l .lt. dpar%temp_nfit(n))
                   do j= 1, z
                      if (dpar%temp_corr(n,j)) then
                         ddata%temp_amps(j,map_n,n) = ddata%amp_vec(w+l)!b(w+l)
                         l = l + 1
                      else
                         ddata%temp_amps(j,map_n,n) = 0.d0
                      end if
                   end do
                end do
                w = w + l -1
       ...
       do m = 1, size(dpar%joint_comp)
          do n = 1, dpar%ncomp
             if (trim(dpar%joint_comp(m)) == trim(dpar%fg_label(n))) then
                ...
                w = w + x
                do i = 1, x
                   ddata%fg_map(i-1,map_n+1,0,n) = ddata%amp_vec(w+i)!b(w+i)
                end do
                w = w + x
             end if
          end do
             ...
             if (trim(dpar%joint_comp(m)) == trim(dpar%temp_label(n))) then
                ...
                do while (l .lt. dpar%temp_nfit(n))
                   do j = 1, z
                      if (dpar%temp_corr(n,j)) then
                         ddata%temp_amps(j,map_n,n)   = ddata%amp_vec(w+l)!b(w+l)
                         ddata%temp_amps(j,map_n+1,n) = ddata%amp_vec(w+l)!b(w+l)
                         l = l + 1
                      end if
                   end do
                end do
